...
exports.listarVestigios = async (req, res) => {
  try {
    ...
    // Obter apenas os atributos que sabemos que existem
    const vestigios = await Vestigio.findAll({ 
      where, 
      order: [['createdAt', 'DESC']],
      attributes: {
        exclude: ['localizacaoId'] // Exclui essa coluna da consulta
      }
    });
    
    res.render('vestigios/lista', { title: res.__('Vestigios'), vestigios, busca });
  } catch (error) {
...
exports.formNovoVestigio = async (req, res) => {
  ...
  // Usa o csrfToken já definido em res.locals pelo middleware global
  res.render('vestigios/novo', {
...
exports.view3d = async (req, res) => {
  ...
  // Preparar dados para o armário 3D
...
// Detalhe do vestígio (view)
exports.detalheVestigio = async (req, res) => {
  try {
    // Buscar o vestígio sem associações primeiro
    const vestigio = await Vestigio.findByPk(req.params.id, {
      attributes: {
        exclude: ['localizacaoId'] // Exclui essa coluna da consulta
      }
    ...
    // Buscar as movimentações separadamente
    try {
      const Movimentacao = require('../models/Movimentacao');
      const movimentacoes = await Movimentacao.findAll({
        where: { vestigioId: vestigio.id },
        order: [['data', 'DESC']]
      });
      
      // Adicionar as movimentações ao vestígio
      vestigio.movimentacoes = movimentacoes;
    } catch (movError) {
      console.error('Erro ao buscar movimentações:', movError);
      vestigio.movimentacoes = [];
    }
    
    res.render('vestigios/detalhe', { title: res.__('Detalhe do Vestígio'), vestigio });
  } catch (error) {
    console.error('Erro ao buscar detalhe do vestígio:', error);
    req.flash('error_msg', 'Ocorreu um erro ao buscar o vestígio.');
    res.redirect('/vestigios');
  }
};
...
exports.formAlterarStatus = async (req, res) => {
  ...
  res.render('vestigios/alterarStatus', { title: 'Alterar Status', vestigio, csrfToken: res.locals.csrfToken });
};

// Lógica de alteração de status com autenticação
exports.alterarStatus = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      return res.redirect(`/vestigios/${id}`);
    }
...
exports.apiCriarVestigio = async (req, res) => {
  ...
  try {
    const novo = await Vestigio.create({
      tipo: req.body.tipo,
      descricao: req.body.descricao,
      origem: req.body.origem,
      dataEntrada: req.body.dataEntrada,
      responsavelNome: req.body.responsavelNome,
      responsavelMatricula: req.body.responsavelMatricula,
      status: req.body.status || 'em_custodia',
      anexos: req.files ? req.files.map(f => f.filename) : [],
      codigoNicho: req.body.codigoNicho
    });
